...
async def doctor_portal():
    ...
    return FileResponse(Path(__file__).parent / "static" / "doctor.html")

@app.get("/patient-portal")
async def patient_portal():
    """Serve the Patient Report Interface"""
...
try:
    ...
    app.include_router(doctor_router, prefix="/doctor", tags=["Doctor Portal"])
    logger.info("✓ Doctor portal loaded")
except ImportError as e:
    logger.warning(f"Doctor portal not loaded: {e}")

# Patient Portal (Report Access)
try:
    from app.api.patient_portal import router as patient_router
    app.include_router(patient_router, prefix="/patient", tags=["Patient Portal"])
    logger.info("✓ Patient portal loaded")
except ImportError as e:
    logger.warning(f"Patient portal not loaded: {e}")
