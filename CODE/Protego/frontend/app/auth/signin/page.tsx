...
export default function SignInPage() {
  ...
  const handleSuccess = (token: string, user: any) => {
    console.log('SignInPage: handleSuccess called with user:', user)
    setUser(user)
    setPendingUser(user)
    console.log('SignInPage: User set in store, showing WhatsApp modal')

    // Check if user has already skipped the modal
    const hasSkipped = localStorage.getItem('whatsapp_sandbox_skipped')
    console.log('SignInPage: hasSkipped value from localStorage:', hasSkipped)

    if (!hasSkipped) {
      // Show WhatsApp sandbox modal
      console.log('SignInPage: Setting showWhatsAppModal to TRUE')
      setShowWhatsAppModal(true)
    } else {
      // Redirect immediately if user has skipped before
      console.log('SignInPage: User previously skipped, redirecting...')
      redirectUser(user)
    }
  }

  const redirectUser = (user: any) => {
    console.log('SignInPage: Redirecting based on user type:', user.user_type)
    setTimeout(() => {
